...
int Command_fetch(apr_pool_t* p, const char* url, int fetch_only)
{
	...
	if (apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
		...
		check(rc == 0, "Git failed to retrive: %s",url);}
		...
		if(info.scheme) {
			...
			check(rc == 0,"Curl failed fetching url: %s",url);}
		else {
			depends_file = info.path;}
        // recurse to process the devpkg list
		...
		return 0;} // Return code 0 means we are done.
		...
		rv = apr_dir_make_recursive(BUILD_DIR, 	APR_UREAD   |
												APR_UWRITE  |
												APR_UEXECUTE, p);
	...
	return 1; //Return code 1 means an install is needed
